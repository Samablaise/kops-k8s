...
# Execute the commands below in your KOPS control Server. use unique s3 bucket name. If you get bucket name exists error.
	...
	export KOPS_STATE_STORE=s3://samatech.k8s.local
...
# 8) Create kubernetes cluster definitions on S3 bucket
	...
	kops create cluster --zones us-west-2c --networking weave --master-size t2.medium --master-count 1 --node-size t2.medium --node-count=2 ${NAME}
...
# 11) connect to the master node
    ...
    ssh -i ~/.ssh/id_rsa ubuntu@54.244.40.186
    ssh -i ~/.ssh/id_rsa ubuntu@172.20.39.188
